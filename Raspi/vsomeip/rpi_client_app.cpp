...
// TC375 코드에서 ULTRASONIC_COUNT가 사용됨. 예시로 4개 가정.
const uint8_t ULTRASONIC_COUNT = 4; 

// =================================================================
// 2. TC375 측에서 사용하는 데이터 구조체 정의 및 크기
//    - vsomeip는 Big Endian을 사용하므로, 리틀 엔디언 환경인 경우 주의해야 함.
// =================================================================
typedef struct
{
    uint8_t id;             // 1
    uint32_t system_time_ms; // 4
    float distance_m;       // 4
    uint8_t distance_status;// 1
    uint16_t signal_strength; // 2
    uint64_t received_time_us;// 8
} ToFData_t;
const size_t TOF_DATA_SIZE = 1 + 4 + 4 + 1 + 2 + 8; // 20 bytes

typedef struct
{
    int32_t dist_raw_mm;    // 4
    int32_t dist_filt_mm;   // 4
    uint64_t received_time_us; // 8
} UltrasonicData_t;
const size_t ULT_DATA_SIZE = 4 + 4 + 8; // 16 bytes

typedef struct
{
    int32_t motorChA_speed; // 4
    int32_t motorChB_speed; // 4
    uint64_t output_time_us;// 8
} MotorControllerData_t;
const size_t MOTOR_DATA_SIZE = 4 + 4 + 8; // 16 bytes

typedef struct
{
    int64_t interval_ms;    // 8
    uint64_t output_time_us;// 8
} EmerAlertData_t;
const size_t EMERALERT_DATA_SIZE = 8 + 8; // 16 bytes

// =================================================================
// 3. vsomeip 전역 변수 및 콜백 함수
// =================================================================
std::shared_ptr<vsomeip::application> app;
vsomeip::client_t client_id = CLIENT_ID;
...
void on_message(const std::shared_ptr<vsomeip::message> &response_msg) {
    ...
    if (service_id == SERVICE_ID_SENSOR) {
        if (method_id == METHOD_ID_TOF && length >= TOF_DATA_SIZE) {
            ...
            if (length > 0) {
                ...
                for (int i = 0; i < count; ++i) {
                    if (offset + ULT_DATA_SIZE <= length) {
        ...
        if (method_id == METHOD_ID_MOTOR && length >= MOTOR_DATA_SIZE) {
        ...
        } else if (method_id == METHOD_ID_ALERT && length >= EMERALERT_DATA_SIZE) {
...
// 모터 제어 요청 함수 (요청 페이로드: int32_t x, int32_t y)
void request_motor_control(int32_t motor_x, int32_t motor_y) {
    ...
    // Payload 구성: Big Endian으로 바이트 변환
    std::vector<vsomeip::byte_t> payload_data;
    
    append_to_payload(payload_data, motor_x); // 4 bytes (rxBuf[16]~[19])
    append_to_payload(payload_data, motor_y); // 4 bytes (rxBuf[20]~[23])
...
// 비상 알림 설정 요청 함수 (요청 페이로드: int64_t cycle_ms)
void request_alert_control(int64_t cycle_ms) {
    ...
    append_to_payload(payload_data, cycle_ms); // 8 bytes (rxBuf[16]~[23])
...
int main(int argc, char **argv) {
    ...
    // 메시지 수신 콜백 등록 (ANY_SERVICE로 두 서비스 ID 모두 처리)
